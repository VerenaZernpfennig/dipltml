...
*/
  ...
  //  ConflictGraph *cg = ConflictGraph::fromConflicts(camera->getConflictIntervals(), camera->getVisibilityIntervals());
    ...
    if (COMPUTE_LARGE_ILPS || (ecg_am1 != nullptr)) {
      ...
      ILPAdapter ilp1(this->map, camera->getVisibilityIntervals(), camera->getConflictIntervals(), Heuristic::AM1, this->k, this->deviation,ilp_filename);
      Clock ilp_clock;
      ilp_clock.start();
      ilp1.run();
    ...
    if (COMPUTE_LARGE_ILPS || (ecg_am2 != nullptr)) {
      ...
      ILPAdapter ilp2(this->map, camera->getVisibilityIntervals(), camera->getConflictIntervals(), Heuristic::AM2, this->k, this->deviation, ilp_filename);
    ...
    if (COMPUTE_LARGE_ILPS || (ecg_am3 != nullptr)) {
      ...
      ILPAdapter ilp3(this->map, camera->getVisibilityIntervals(), camera->getConflictIntervals(), Heuristic::AM3, this->k, this->deviation, ilp_filename);
