...
bool ILP::execute(Instance* instance, ModelType modelType, vector<Intervals> & result,
							double &resultBound, double &gap, Clock & clock, int k, ILPAdapter *adapter, const char *dbg_filename, int deviation) {
		...
		//	const string temp = name + "_D";
			...
			case CONTAINED:
				...
				if(endSegment == endPresence){                   
                    model.addConstr(sumB <= deviation, "c6_"+name);
                    model.addConstr(sumE <= deviation, "c7_"+name); 
				...
				if( endSegment != endPresence){
					...
					if(modelType == AM2 || modelType == AM3 || modelType == AM4){
				...
				if(beginSegment != beginPresence){
					...
					witnessBegin += 0;
                                        ...
                                        if(modelType == AM3 || modelType == AM4){
