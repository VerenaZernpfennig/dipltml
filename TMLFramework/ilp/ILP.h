...
inline long timevaldiff(struct timeval *starttime, struct timeval *finishtime)
{
  ...
  return msec;
	...
	enum ModelType {AM1, AM2, AM3,AM4};
class ILP {
public:
	...
	bool execute(Instance * instance, ModelType modelType, vector<Intervals> & result, double &resultBound, double &gap, Clock & clock, int k, ILPAdapter *adapter, 
    const char *dbg_filename, int deviation);
