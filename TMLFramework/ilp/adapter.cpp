...
ILPAdapter::ILPAdapter(Map *map, VisibilityIntervals &visibilityIntervals, ConflictIntervals &conflicts, Heuristic::ModelType mtype, int k, int deviation, const char* dbg_filename)
  : map(map), vi(visibilityIntervals), ci(conflicts), labelIntervals(nullptr), mtype(mtype), k(k), deviation(deviation), dbg_filename(dbg_filename)
{}
...
void ILPAdapter::run()
{
  ...
  try {
    ...
    switch (this->mtype) {
      case Heuristic::AM1:
        timeLimitExeeded = ! ilp.execute(&(this->instance), ILP::AM1, result, this->resBound, this->gap, clock, this->k, this, this->dbg_filename, this->deviation);
        break;
      case Heuristic::AM2:
        timeLimitExeeded = ! ilp.execute(&(this->instance), ILP::AM2, result, this->resBound, this->gap, clock, this->k, this, this->dbg_filename, this->deviation);
        break;
      case Heuristic::AM3:
        timeLimitExeeded = ! ilp.execute(&(this->instance), ILP::AM3, result, this->resBound, this->gap, clock, this->k, this, this->dbg_filename,this->deviation);
        break;
       case Heuristic::AM4:
        timeLimitExeeded = ! ilp.execute(&(this->instance), ILP::AM4, result, this->resBound, this->gap, clock, this->k, this, this->dbg_filename,this->deviation);
        break; 
    }
