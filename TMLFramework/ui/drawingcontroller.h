...
public:
    ...
    void new_route(int seed,double speed,int startpoint, int endpoint);
    ...
    void compute_heuristic(int heuristic_tag, int k = -1, int deviation= 1);

    void save_camera(const char *filename);
    
    void save_camera_image(const char *filename);
...
private:
    ...
    void draw_trajectory_cs();
    QGraphicsItem *draw_trajectory_straight(StraightTrajectoryItem *i);
    QGraphicsItem *draw_trajectory_straight_cs(StraightTrajectoryItem *i);
    QGraphicsItem *draw_trajectory_circle(CircleTrajectoryItem *i);
    QGraphicsItem *draw_trajectory_circle_cs(CircleTrajectoryItem *i);
