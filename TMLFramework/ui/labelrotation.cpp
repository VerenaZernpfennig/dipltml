...
{
  this->controller.new_route(this->sinfo->seed,this->sinfo->speed,this->sinfo->startpoint,this->sinfo->endpoint);
  this->controller.compute_heuristic(this->sinfo->heuristic, this->sinfo->k, this->ui->deviationBox->value());
  // set size
  /*
  int total_size = 0;
    ...
    sinfo(sinfo)
{
    ...
    ui->heuristicComboBox->insertItem(6, "ILP AM4", QVariant(HEU_ILP_AM4_TAG));
...
{
    this->controller.new_route(this->ui->seedSpinBox->value(),this->ui->speedSpinBox->value(),this->ui->routeStartBox->value(),this->ui->routeEndBox->value());
    this->counter = 0;
}
...
void LabelRotation::on_stopButton_clicked()
{
    this->controller.stop_play();
}
...
void LabelRotation::on_stepSpinner_valueChanged(double arg1)
{
  ...
  //save camera view to session folder
  this->cameraView->setSizePolicy(QSizePolicy::Fixed,QSizePolicy::Fixed);
  this->cameraView->resize(CAMERA_WIDTH, CAMERA_HEIGHT);
  this->controller.update();
  std::cout << "Counting number ---- " << counter << "\n";
  //Save file to specific file path
  std::string filename = "/home/verena/Desktop/Session/Pic_" + std::to_string(counter) + ".png";
  std::cout << "filename generated: " + filename;
  std::ofstream ofile(filename);
  this->controller.save_camera_image(filename.c_str());
  this->cameraView->setSizePolicy(QSizePolicy::Ignored,QSizePolicy::Ignored); 
}
...
void LabelRotation::on_stepSlider_valueChanged(int value)
{
  ...
  this->cameraView->resize(CAMERA_WIDTH, CAMERA_HEIGHT);
  this->controller.update();
  std::cout << "Counting number ---- " << value << "\n";
  //Save file to specific file path
  std::string filename = "/home/verena/Desktop/Session/Pic_" + std::to_string(value) + ".png";
  std::cout << "filename generated: " + filename;
  std::ofstream ofile(filename);
  this->controller.save_camera_image(filename.c_str());
  this->cameraView->setSizePolicy(QSizePolicy::Ignored,QSizePolicy::Ignored); 
...
void LabelRotation::on_runHeuristicButton_clicked()
{
    ...
    this->controller.compute_heuristic(tag,-1,this->ui->deviationBox->value());
}

void LabelRotation::on_saveButton_clicked()
{
     this->cameraView->setSizePolicy(QSizePolicy::Fixed,
QSizePolicy::Fixed);
     this->cameraView->resize(CAMERA_WIDTH, CAMERA_HEIGHT);
     this->controller.update();
     this->controller.save_camera("/tmp/screenshot.svg");
     this->controller.save_camera_image("/tmp/screenshot.png");
     this->cameraView->setSizePolicy(QSizePolicy::Ignored,
QSizePolicy::Ignored);
}
